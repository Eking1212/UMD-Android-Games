...
public class PuzzleState implements State {
    ...
    PuzzleState(State start, int i, int i0) {
...
/*        @Override
 ...
 //       return tilesOutOfPlace(this, goal);
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        ...
        int count = 0;
        for (int i = 1; i<9;i++){
            if(!currentState.getLocation(i).equals(goalState.getLocation(i))){
                count ++;
            }
        } 
        return count;
...
//            if(currentState.tiles[row][col]!=(goalState.tiles[row][col])){
            for(int i =1; i<9; i++){
                sum += findTile(goalState,currentState,i);
            }
   ...
   /* public static int findTile(PuzzleState goal,PuzzleState curr, int tile){
        int differenceCol = 0;
        int differenceRow = 0;
        int sum = 0;
        
        differenceRow = Math.abs(goal.getLocation(tile).getRow() - curr.getLocation(tile).getRow());
        differenceCol = Math.abs(goal.getLocation(tile).getColumn() - curr.getLocation(tile).getColumn());
        //curr.getLocation(tile).getRow();
        //curr.getLocation(tile).getColumn();
//        System.out.println(sum);
        sum = differenceRow+differenceCol;
        System.out.println(sum);
